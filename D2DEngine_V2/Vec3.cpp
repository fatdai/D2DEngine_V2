...
Vec3::Vec3(const float *array) { set(array); }

Vec3::Vec3(const Vec3 &p1, const Vec3 &p2) { set(p1, p2); }
...
bool Vec3::isOne() const { return x == 1.0f && y == 1.0f && z == 1.0f; }
...
float Vec3::angle(const Vec3 &v1, const Vec3 &v2) {
  ...
  return atan2f(sqrt(dx * dx + dy * dy + dz * dz) + MATH_FLOAT_SMALL,
                dot(v1, v2));
}
...
void Vec3::cross(const Vec3 &v) { cross(*this, v, this); }

void Vec3::cross(const Vec3 &v1, const Vec3 &v2, Vec3 *dst) {
  assert(dst);

  // NOTE: This code assumes Vec3 struct members are contiguous floats in
  // memory.
