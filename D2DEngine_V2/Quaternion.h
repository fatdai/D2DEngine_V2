...
/**
 * Defines a 4-element quaternion that represents the orientation of an object
 *in space.
 *
 * Quaternions are typically used as a replacement for euler angles and rotation
 *matrices as a way to achieve smooth interpolation and avoid gimbal lock.
 *
 ...
 * This class provides three methods for doing quaternion interpolation: lerp,
 *slerp, and squad.
 *
 * lerp (linear interpolation): the interpolation curve gives a straight line in
 *quaternion space. It is simple and fast to compute. The only problem is that
 *it does not provide constant angular velocity. Note that a constant velocity
 *is not necessarily a requirement for a curve;
 * slerp (spherical linear interpolation): the interpolation curve forms a great
 *arc on the quaternion unit sphere. Slerp provides constant angular velocity;
 * squad (spherical spline interpolation): interpolating between a series of
 *rotations using slerp leads to the following problems:
 ...
 *
 * Since squad is continuously differentiable, it remedies the first and third
 ...
 *liberty to use the negative of either input. The resulting path is always the
 *shorter arc.
 *
 * The lerp method provided here interpolates strictly in quaternion space. Note
 *that the resulting path may pass through the origin if interpolating between a
 *quaternion and its exact negative.
 *
 ...
 * q4 = (-0.8, 0.0, -0.6, 0.0).
 * For the point p = (1.0, 1.0, 1.0), the following figures show the
 *trajectories of p using lerp, slerp, and squad.
 */
class Quaternion {
  friend class Curve;
...
public:
  ...
  /**
   * Constructs a quaternion equal to the rotational part of the specified
   *matrix.
   *
   * @param m The matrix.
   */
  ...
  /**
   * Constructs a quaternion equal to the rotation from the specified axis and
   *angle.
   *
  ...
  /**
   * Creates this quaternion equal to the rotation from the specified axis and
   *angle
   * and stores the result in dst.
   *
   * @param axis A vector describing the axis of rotation.
   ...
   * @param dst A quaternion to store the conjugate in.
   */
  static void createFromAxisAngle(const Vec3 &axis, float angle,
                                  Quaternion *dst);
  ...
  /**
   * Multiplies this quaternion by the specified one and stores the result in
   *this quaternion.
   *
   ...
   */
  static void multiply(const Quaternion &q1, const Quaternion &q2,
                       Quaternion *dst);
   ...
   *
   * @param array An array containing the elements of the quaternion in the
   *order x, y, z, w.
   */
  ...
  /**
   * Sets the quaternion equal to the rotation from the specified axis and
   *angle.
   *
   ...
   * @param dst A quaternion to store the result in.
   */
  static void lerp(const Quaternion &q1, const Quaternion &q2, float t,
                   Quaternion *dst);
   ...
   *
   * Note: For accurate interpolation, the input quaternions must be at (or
  ...
  /**
   * Interpolates over a series of quaternions using spherical spline
   *interpolation.
   *
   * Spherical spline interpolation provides smooth transitions between
   *different
   * orientations and is often useful for animating models or cameras in 3D.
   *
   ...
   * so it is up to the caller to ensure they call normalize beforehand, if
   *necessary.
   *
   * @param q1 The first quaternion.
   * @param q2 The second quaternion.
  ...
  static void squad(const Quaternion &q1, const Quaternion &q2,
                    const Quaternion &s1, const Quaternion &s2, float t,
                    Quaternion *dst);
...
private:
   ...
   */
  static void slerp(float q1x, float q1y, float q1z, float q1w, float q2x,
                    float q2y, float q2z, float q2w, float t, float *dstx,
                    float *dsty, float *dstz, float *dstw);

  static void slerpForSquad(const Quaternion &q1, const Quaternion &q2, float t,
                            Quaternion *dst);
};
}
