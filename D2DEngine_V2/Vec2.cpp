...
// returns true if segment A-B intersects with segment C-D. S->E is the ovderlap
// part
bool isOneDimensionSegmentOverlap(float A, float B, float C, float D, float *S,
                                  float *E) {
...
float crossProduct2Vector(const Vec2 &A, const Vec2 &B, const Vec2 &C,
                          const Vec2 &D) {
...
bool Vec2::isZero() const { return x == 0.0f && y == 0.0f; }

bool Vec2::isOne() const { return x == 1.0f && y == 1.0f; }
...
bool Vec2::isLineIntersect(const Vec2 &A, const Vec2 &B, const Vec2 &C,
...
bool Vec2::isLineParallel(const Vec2 &A, const Vec2 &B, const Vec2 &C,
  ...
  if (crossProduct2Vector(A, B, C, D) == 0) {
    // line overlap
    if (crossProduct2Vector(C, D, C, A) == 0 ||
        crossProduct2Vector(A, B, C, A) == 0) {
      return false;
    }
...
bool Vec2::isLineOverlap(const Vec2 &A, const Vec2 &B, const Vec2 &C,
                         const Vec2 &D) {
  // FAIL: Line undefined
  if ((A.x == B.x && A.y == B.y) || (C.x == D.x && C.y == D.y)) {
    return false;
...
bool Vec2::isSegmentOverlap(const Vec2 &A, const Vec2 &B, const Vec2 &C,
...
bool Vec2::isSegmentIntersect(const Vec2 &A, const Vec2 &B, const Vec2 &C,
                              const Vec2 &D) {
  float S, T;
...
Vec2 Vec2::getIntersectPoint(const Vec2 &A, const Vec2 &B, const Vec2 &C,
                             const Vec2 &D) {
  float S, T;
