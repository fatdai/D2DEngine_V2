...
inline float clampf(float value, float min_inclusive, float max_inclusive) {
  ...
  return value < min_inclusive ? min_inclusive
...
public:
   ...
   *
   * @param array An array containing the elements of the vector in the order x,
   *y.
   */
  Vec2(const float *array);

  /**
   * Constructs a vector that describes the direction between the specified
   *points.
   *
  ...
  /**
   * Clamps the specified vector within the specified range and returns it in
   *dst.
   *
   ...
   * response time yields a smoother result and more lag. To force this vector
   *to
   * follow the target closely, provide a response time that is very small
   *relative
   ...
   * @param v The vector to compare against.
   *
   * @return True if this vector is not equal to the given vector, false
   *otherwise.
   */
  inline bool operator!=(const Vec2 &v) const;
 ...
 */
   ...
   * @lua NA
   */
  inline Vec2 getClampPoint(const Vec2 &min_inclusive,
                            const Vec2 &max_inclusive) const {
    return Vec2(clampf(x, min_inclusive.x, max_inclusive.x),
                clampf(y, min_inclusive.y, max_inclusive.y));
  }
   ...
   * @lua NA
   */
  static bool isLineIntersect(const Vec2 &A, const Vec2 &B, const Vec2 &C,
                              const Vec2 &D, float *S = nullptr,
                              float *T = nullptr);
   ...
   * @lua NA
   */
  static bool isLineOverlap(const Vec2 &A, const Vec2 &B, const Vec2 &C,
                            const Vec2 &D);
   ...
   * @lua NA
   */
  static bool isLineParallel(const Vec2 &A, const Vec2 &B, const Vec2 &C,
                             const Vec2 &D);
   ...
   * @lua NA
   */
  static bool isSegmentOverlap(const Vec2 &A, const Vec2 &B, const Vec2 &C,
                               const Vec2 &D, Vec2 *S = nullptr,
                               Vec2 *E = nullptr);
   ...
   * @lua NA
   */
  static bool isSegmentIntersect(const Vec2 &A, const Vec2 &B, const Vec2 &C,
                                 const Vec2 &D);
   ...
   * @lua NA
   */
  static Vec2 getIntersectPoint(const Vec2 &A, const Vec2 &B, const Vec2 &C,
                                const Vec2 &D);
